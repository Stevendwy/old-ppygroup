...
export default class ShowImageView extends Component {
	...
	componentDidMount() {
		...
		catchEvent(middleEvents.floatwindowSubGroupClick, () => {
			this.setState({
	...
	areaClick(code, e) {
		setTimeout(() => {
				$(".MiddleRightContentListItemContainer").css('border', "none");
				$('.chooseAndFloatItem>div').css("border", "2px solid #3385ff");
			}, 10)
			//通知右侧点击了
	...
	imgMouseEvent(e) {
		...
		if (_type == "mousedown" || _type == "touchstart") {
			_img.src = this.hasTransform ? cdnHost + "/img/icon_zaidian.png" : cdnHost + "/img/icon_kuandian.png"
		} else if (_type == "mouseup" || _type == "touchend") {
			_img.src = this.hasTransform ? cdnHost + "/img/icon_zai.png" : cdnHost + "/img/icon_zai.png"
			this.transform()
		}
	}
	...
	loadError(e) {
		if (this.props.imgsData.data.imgurl.length > 0) {
			e.target.src = "/img/util_opacity.png"
		...
		if (isVIN) _detailContainer = (
			...
			<div>
					<div className="DetailContainer"
						...
						>车辆配置</p></div>
					</div> < div className = "CarMIContainer"
			onClick = {
				() => {
					this.setState({
						whitchTitel: "guanjian"
					}, () => {
			...
			onMouseOver = {
				() => this.refs.controlbubbledetailp1.style.color = "#45C0FF"
			}
			onMouseLeave = {
				() => this.refs.controlbubbledetailp1.style.color = "#333333"
			} >
			...
			<div className="ShowImageViewContainer"
				...
				<div ref="showimageview" className="ShowImageView">
					...
					<div ref="showimageviewerror" className="ShowImageViewError">
						<img src={cdnHost+"/img/p_jiegoutu.png"} />
