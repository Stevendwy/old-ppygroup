	...
	changeIndex(index,whitch){
		...
		this.setState({
			...
			if (whitch=="渠道价格") {
				...
				this.flag=false;
				let _replaceurl = "/ppys/partreplace"
//				 if(_part.brand ==  ){ /ppys/partreplace
//				 	_replaceurl = "/ppys/replacedetail"
//				 }
				getAjax(_replaceurl, _obj, response => {
						this.setState({
							clickIndex:index,
	...
	render(){
		...
		let _contentlist=_content.map((item,index)=>{
			return (
				<div key={index}
					className="GroupPartDetailListModelContentcontentLi" style={{marginLeft: "7px"}}>
					{item} 
				</div>
			)
		})
