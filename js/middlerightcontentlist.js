...
export default class MiddleRightContentList extends Component {
	...
	constructor(props) {
		...
		this.isRedVinFilter = false //处理点击vin过滤，执行两次请求bug
	}
	...
	componentDidMount() {
		...
		catchEvent(middleEvents.redVinFilter, (e) => {
			if (!isVIN) return
			this.isRedVinFilter = true
				// console.log("redVinFilter")
			...
			getAjax(_url, _obj, response => {
				...
				this.props.setParts(_data)
				this.response = response
				this.hash = []
				this.setState({
					datas: _data,
					flag: response.flage,
					displayNone: _data.length > 0 ? false : true,
					selectIndex: -1,
					floatItemIndex: -1,
					sameItids: []
				}, () => {
		...
		if (isFirstFirer) {
			...
			getAjax("/userhabits", {}, res => {
				if (res.data.vin_filter == "1") {
					// this.titlechange("vinguolv")
					this.auth = e.info ? e.info.auth : this.auth //下面一样用到
					sendEvent(middleEvents.isHasChangeType)
					return
				} else {
					...
					this.setState({
						...
						getAjax(_url, _obj, response => {
							// console.log("subgroupclick")
							// console.log(this.uid);
							if (response.uid != this.uid) return
							//console.log(JSON.stringify(response))
							this.props.setParts(response.data)
							this.response = response
							this.hash = []
							this.setState({
								datas: response.data,
	...
	render() {
		...
		if (this.state.isFilter && isVIN) _warning = (<div className="MiddleRightContentListWarningContainer">
			<div className="MiddleRightContentListWarningRemuse" style={{color:"#FF3232"}}>*红色字体：未经VIN过滤的数据</div>
		</div>)
		if (_notRunTime) _warning = (
			<div className="MiddleRightContentListWarningContainer">
                ...
                {/*
                    ...
                    <img className="MiddleRightContentListWarningImg" src="/img/icon_shuaxin.png" alt="warning" />
